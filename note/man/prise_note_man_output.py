...
VARS_NOTE = {
    VAR_ESTIMATE: dict(
        label = "AVC",
        value = ESTIMATE_TRUE + [ESTIMATE_FALSE]
    ),
    VAR_COMMENT: dict(
        label = "COMMENTAIRE",
        value = EMPTY
...
def data_path(mode) :
    return f"{WK_DIR}/data/{SPLIT}/{SPLIT}_{ID}_note_man_data_{mode}{DATA_SUFFIX}.csv"

data_man_input = data_path("input")
data_man_output = data_path("output")

def safe_read_csv(data, sep = SEP, **kwargs):
    return pd.read_csv(data, sep=sep, keep_default_na = False, **kwargs)

dtypes = {
    col: str for col in safe_read_csv(data=data_man_input).columns
    if col.startswith("id_")
}

@st.cache_data
def read_data():
    return safe_read_csv(data=data_man_input, dtype=dtypes)
...
if os.path.exists(data_man_output):
    ...
    df_output = safe_read_csv(
        data = data_man_output,
        sep = ",",
        dtype = dtypes
    )
    ...
    any_estimate_value = check_note_values(
        data = df_output,
        var = VAR_ESTIMATE_PREFIX,
        value = VARS_NOTE[VAR_ESTIMATE]["value"]
    )
...
df_output.to_csv(data_man_output, index=False)
...
with col_text :
    ...
    html(
        html = load_css("style_text") + text_input,
        scrolling = True
    )
...
with col_df :
    ...
    df_note = df_output[["n", "id_ipp", "id_iep"] + list(VARS_NOTE)]
    ...
    event = st.dataframe(
        ...
        column_config = {
            "n": st_col(label = EMPTY, width = 40),
            **{
                name: st_col(label = VARS_NOTE[name]["label"])
                for name in VARS_NOTE
            }
        },
...
with st.sidebar.container(key = f"button-download-{DOWNLOAD}"):
    download(
        secrets = SECRETS,
        export_data = (
            df_output[["n"] + list(VARS_META) + list(VARS_NOTE)]
            .query(f"{VAR_ESTIMATE} not in ['{ESTIMATE_FALSE}', '{EMPTY}']")
        ),
